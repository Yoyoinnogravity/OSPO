#!/usr/bin/env node
/**
 * Node checks for preplot inference (no Confirm Columns default).
...
assert(p1Parsed.confidence !== 'low' && p1Parsed.confidence !== 'none', 'P1 confidence should not force mapper: ' + p1Parsed.confidence);
...
assert(resultIsAutoLoad(clear) === true, 'clear unlabeled UTM should auto-map, conf=' + clear.confidence);
...
assert(resultIsAutoLoad(amb) === false || (amb.layout && amb.layout.confidence < 0.45),
  'ambiguous unlabeled should ask (not auto). conf=' + amb.confidence + ' layoutConf=' + (amb.layout && amb.layout.confidence));
...
assert(resultIsAutoLoad(trap) === true, 'comma-in-H P1 must auto-load as P1, not CSV confirm. conf=' + trap.confidence + ' fmt=' + trap.format);
...
console.log(JSON.stringify({
  ...
  namedCsv: { lines: named.lines.length, conf: named.confidence },
  unlabeledClear: { lines: clear.lines.length, conf: clear.confidence },
  unlabeledAmbiguous: { conf: amb.confidence, cols: table.headerCols.length, sampleWidth: (table.sampleRows[0] || []).length, auto: resultIsAutoLoad(amb) },
}, null, 2));
process.exit(0);
