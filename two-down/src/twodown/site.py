...
from twodown.config import BRAND, SITE_ORIGIN, SITE_ROOT
...
<body>
  ...
  <footer>
    ...
    Not affiliated with those papers. Voice: Sonia.
  </footer>
...
def _article(item: SpokenClue, media_prefix: str, open_by_default: bool = False) -> str:
    ...
    video = ""
    if item.video_path:
        video = f'<video controls playsinline src="{_e(media_prefix + clue.slug + ".mp4")}"></video>'
    enum = f" ({_e(clue.enumeration)})" if clue.enumeration else ""
    return f"""
    <article class="clue{opened}">
...
def publish_site(pair: DailyPair, dest: Path | None = None) -> Path:
    ...
    about = """
    <h1>About.</h1>
    <p class="lede">cryptic.fun publishes two cryptic clues a day, taken from the Fifteen Squared blogs of the Independent, Guardian and Financial Times. Sonia reads the parse. YouTube gets the Short. The site is the spoiler-safe home.</p>
